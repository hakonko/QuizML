...
class DashboardApp(QWidget):
    def __init__(self, user: User, user_db: UserDatabase, quiz_callback, retake_callback):
        super().__init__()
        self.user = user
        ...
        self.setStyleSheet("background-color: black;")  # Set background to black for the entire window

        layout = QHBoxLayout()
        layout.setContentsMargins(20, 20, 20, 20)  # eller 30, 20, 30, 20 for mer kontroll
        layout.setSpacing(20)  # for luft mellom venstre og høyre



        # Left side (set margin and spacing to zero)
        ...
        for btn in [self.new_quiz_btn, self.retake_btn, self.delete_btn]:
            btn.setStyleSheet(
        ...
        # Right side (set margin and spacing to zero)
        ...
        self.num_problems_slider.setValue(10)  # Default value
    ...
    def refresh_quiz_list(self):
        ...
        # Sorter quizzene med nyeste først
        ...
        for idx, quiz in enumerate(sorted_quizzes):
            ...
            item = QListWidgetItem(f"Quiz {idx + 1} ({percent}%) – {quiz_date} ({num_questions} questions)")
